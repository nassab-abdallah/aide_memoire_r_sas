...
Les boucles ont généralement lieu au sein d'une macro.
...
Il existe 2 grandes stratégies pour construire des boucles en `R` :

1. Les boucles `for`

Leur utilisation est basique :
...
- En cas d'utilisation d'un conteneur, il est judicieux de le préallouer pour des questions d'optimisation de la mémoire dans `R` :

```{r, warning=FALSE, message=FALSE, results='hide'}
# Permet de pré-allouer une liste pour y ajouter des données
conteneur <- vector("list", 10)
for (i in 1:10) {
  conteneur[[i]] <- i
}
```

- Le symbole `:` fonctionne à la fois avec des séquences croissantes, mais aussi décroissantes, ce qui peut être source d'erreurs.

```{r, warning=FALSE, message=FALSE, results='hide'}
nb <- 0
conteneur <- vector("list", nb)
for (i in 1:nb) print(i)
# Erreur
# for (i in 1:nb) conteneur[[i]] <- i
```

Il vaut mieux utiliser `seq_along()`, qui ne fonctionne qu'en sens croissant.
...
- La boucle `for` peut supprimer les attributs des éléments lors de l'itération. Dans l'exemple suivant, le formatage des dates est supprimé lors de la boucle :

```{r, warning=FALSE, message=FALSE, results='hide'}
valeurs <- as.Date(c("2020-01-01", "2010-01-01"))
for (i in valeurs) {
  print(i)
}
```

Il vaut mieux alors utiliser le code suivant, qui conserve les attributs :

```{r, warning=FALSE, message=FALSE, results='hide'}
valeurs <- as.Date(c("2020-01-01", "2010-01-01"))
for (i in seq_along(valeurs)) {
  print(valeurs[[i]])
}
```

2. Les fonctions de la famille `apply` 

```{r, warning=FALSE, message=FALSE, results='hide'}
# Sous forme de liste
purrr::map(1:10, print)
# Sous forme de vecteur
purrr::map_int(1:10, print)
```
